...
def DDx(f, h):
    ...
    DDx_f[0, :, :] = (2*f[0, :, :] - 5*f[1, :, :] + 4*f[2, :, :] - f[3, :, :])/(h**2)
    DDx_f[-1, :, :] = (2*f[-1, :, :] - 5*f[-2, :, :] + 4*f[-3, :, :] - f[-4, :, :])/(h**2)
    return DDx_f 

def DDy(f, h):
    DDy_f = np.zeros_like(f)
    DDy_f[:, 1:-1, :] = (f[:, 2:, :] - 2*f[:, 1:-1, :] + f[:, :-2, :])/(h**2)
    DDy_f[:, 0, :] = (2*f[:, 0, :] - 5*f[:, 1, :] + 4*f[:, 2, :] - f[:, 3, :])/(h**2)
    DDy_f[:, -1, :] = (2*f[:, -1, :] - 5*f[:, -2, :] + 4*f[:, -3, :] - f[:, -4, :])/(h**2)
    return DDy_f 

def DDz(f, h):
    DDz_f = np.zeros_like(f)
    DDz_f[:, :, 1:-1] = (f[:, :, 2:] - 2*f[:, :, 1:-1] + f[:, :, :-2])/(h**2)
    DDz_f[:, :, 0] = (2*f[:, :, 0] - 5*f[:, :, 1] + 4*f[:, :, 2] - f[:, :, 3])/(h**2)
    DDz_f[:, :, -1] = (2*f[:, :, -1] - 5*f[:, :, -2] + 4*f[:, :, -3] - f[:, :, -4])/(h**2)
    return DDz_f 
