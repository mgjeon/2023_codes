...
def calculate_derivative(Bx, By, Bz, dx, dy, dz):
    ...
    return B, norm_B, J, norm_J, JxB, norm_JxB, div_B, norm_div_B, laplacian_B, norm_laplacian_B, energy_density_B

def calculate_metric(dx, dy, dz, Lx, Ly, Lz, B, norm_B, J, norm_J, JxB, norm_JxB, div_B, norm_div_B, laplacian_B, norm_laplacian_B, energy_density_B):
    dV = dx*dy*dz # Mm^3
    V = Lx*Ly*Lz # Mm^3
    ...
    return total_energy, loss_force_free, loss_force_free_mean, loss_div_free, loss_div_free_mean, \
           sigma_J, theta_J, theta_i_mean, norm_laplacian_B_mean, \
           max_idx_0, norm_laplacian_B_max_0, max_idx_1, norm_laplacian_B_max_1, max_idx_2, norm_laplacian_B_max_2


def draw_projection(array3D, title, save_dir, dx, dy, dz, Lx, Ly, Lz, z_Mm, cm=False, log=True, cmap='viridis'):
...
def evaluate_single(nc_file, *args):
    #------------------------------------------------------------
    obs_date = os.path.basename(nc_file).split('.')[0][6:].replace('_', 'T') # YYYYMMDD_HHMMSS (string)

    result_dir = os.path.join(args[0].result_dir, obs_date) # result_dir / YYYYMMDD_HHMMSS
    ...
    B, norm_B, J, norm_J, JxB, norm_JxB, div_B, norm_div_B, laplacian_B, norm_laplacian_B, energy_density_B \
    = calculate_derivative(Bx, By, Bz, dx, dy, dz)

    B_pot, norm_B_pot, J_pot, norm_J_pot, JxB_pot, norm_JxB_pot, div_B_pot, norm_div_B_pot, laplacian_B_pot, norm_laplacian_B_pot, energy_density_B_pot \
    ...
    total_energy, loss_force_free, loss_force_free_mean, loss_div_free, loss_div_free_mean, \
    sigma_J, theta_J, theta_i_mean, norm_laplacian_B_mean, \
    max_idx_0, norm_laplacian_B_max_0, max_idx_1, norm_laplacian_B_max_1, max_idx_2, norm_laplacian_B_max_2 =  calculate_metric(dx, dy, dz, Lx, Ly, Lz, B, norm_B, J, norm_J, JxB, norm_JxB, div_B, norm_div_B, laplacian_B, norm_laplacian_B, energy_density_B)

    total_energy_pot, loss_force_free_pot, loss_force_free_mean_pot, loss_div_free_pot, loss_div_free_mean_pot, \
    sigma_J_pot, theta_J_pot, theta_i_mean_pot, norm_laplacian_B_mean_pot, \
    max_idx_0_pot, norm_laplacian_B_max_0_pot, max_idx_1_pot, norm_laplacian_B_max_1_pot, max_idx_2_pot, norm_laplacian_B_max_2_pot =  calculate_metric(dx, dy, dz, Lx, Ly, Lz, B_pot, norm_B_pot, J_pot, norm_J_pot, JxB_pot, norm_JxB_pot, div_B_pot, norm_div_B_pot, laplacian_B_pot, norm_laplacian_B_pot, energy_density_B_pot)
    ...
    draw_projection(energy_density_B, "Energy density (erg/cm^3)", os.path.join(result_dir, 'energy.png'), dx, dy, dz, Lx, Ly, Lz, z_Mm, cm=True, log=False, cmap='jet')
    draw_projection(energy_density_B_pot, "Energy density pot (erg/cm^3)", os.path.join(result_dir, 'energy_pot.png'), dx, dy, dz, Lx, Ly, Lz, z_Mm, cm=True, log=False, cmap='jet')
    draw_projection(free_energy_density, "Free energy density (erg/cm^3)", os.path.join(result_dir, 'energy_free.png'), dx, dy, dz, Lx, Ly, Lz, z_Mm, cm=True, log=False, cmap='jet')
    
    draw_projection(norm_J, r"$|\nabla \times \mathbf{B}|$" + " (G/Mm)", os.path.join(result_dir, 'J.png'), dx, dy, dz, Lx, Ly, Lz, z_Mm)
    draw_projection(norm_laplacian_B, r"$|\nabla^2 \mathbf{B}|$" + " (G/Mm^2)", os.path.join(result_dir, 'laplacian_B.png'), dx, dy, dz, Lx, Ly, Lz, z_Mm)
    draw_projection(norm_JxB, r"$|\mathbf{J} \times \mathbf{B}|$" + " (G^2/Mm)", os.path.join(result_dir, 'JxB.png'), dx, dy, dz, Lx, Ly, Lz, z_Mm)
    draw_projection(norm_div_B, r"$|\nabla \cdot \mathbf{B}|$" + " (G/Mm)", os.path.join(result_dir, 'div_B.png'), dx, dy, dz, Lx, Ly, Lz, z_Mm)

    draw_projection(norm_J_pot, r"$|\nabla \times \mathbf{B}|$" + " pot (G/Mm)", os.path.join(result_dir, 'J_pot.png'), dx, dy, dz, Lx, Ly, Lz, z_Mm)
    draw_projection(norm_laplacian_B_pot, r"$|\nabla^2 \mathbf{B}|$" + " pot (G/Mm^2)", os.path.join(result_dir, 'laplacian_B_pot.png'), dx, dy, dz, Lx, Ly, Lz, z_Mm)
    draw_projection(norm_JxB_pot, r"$|\mathbf{J} \times \mathbf{B}|$" + " pot (G^2/Mm)", os.path.join(result_dir, 'JxB_pot.png'), dx, dy, dz, Lx, Ly, Lz, z_Mm)
    draw_projection(norm_div_B_pot, r"$|\nabla \cdot \mathbf{B}|$" + " pot (G/Mm)", os.path.join(result_dir, 'div_B_pot.png'), dx, dy, dz, Lx, Ly, Lz, z_Mm)
