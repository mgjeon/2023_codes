...
from tool.nf2.train.model import VectorPotentialModel
...
def load_cube(save_path, device=None, z=None, strides=1, **kwargs):
    ...
    coords = np.stack(
        np.mgrid[: cube_shape[0] : strides, : cube_shape[1] : strides, :z:strides], -1
    )
    return load_coords(
        model,
        cube_shape,
...
def load_height_surface(
    save_path, device=None, strides=1, batch_size=1000, progress=False, **kwargs
):
    if device is None:
        device = (
            torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
...
def load_height_cube(save_path, *args, device=None, strides=1, **kwargs):
    ...
    slice = load_height_surface(
        save_path, *args, device=device, strides=strides, **kwargs
    )
...
def load_coords(
    ...
    assert np.all(coords[..., 0] < cube_shape[0]), (
        "Invalid x coordinate, maximum is %d" % cube_shape[0]
    )
    assert np.all(coords[..., 1] < cube_shape[1]), (
        "Invalid x coordinate, maximum is %d" % cube_shape[1]
    )
    assert np.all(coords[..., 2] < cube_shape[2]), (
        "Invalid x coordinate, maximum is %d" % cube_shape[2]
    )
    ...
    if isinstance(model, VectorPotentialModel) or (
        ...
        and isinstance(model.module, VectorPotentialModel)
    ):
        return _load(coords)
    else:
        with torch.no_grad():
