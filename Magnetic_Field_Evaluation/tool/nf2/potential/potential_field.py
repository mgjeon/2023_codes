...
class PotentialModel(nn.Module):
    ...
    def forward(self, coord):
        ...
        v2 = (
            2
            * np.pi
            * ((-self.r_p[:, None] + coord[None, :] + self.c[None]) ** 2).sum(-1) ** 0.5
        )
...
def get_potential_boundary(b_n, height, batch_size=2048, **kwargs):
    ...
    coords = [
        np.stack(
            np.mgrid[
                : cube_shape[0], : cube_shape[1], cube_shape[2] - 2 : cube_shape[2] + 1
            ],
            -1,
        ),
        ...
        np.stack(
            np.mgrid[
                : cube_shape[0], cube_shape[1] - 2 : cube_shape[1] + 1, : cube_shape[2]
            ],
            -1,
        ),
        ...
        np.stack(
            np.mgrid[
                cube_shape[0] - 2 : cube_shape[0] + 1, : cube_shape[1], : cube_shape[2]
            ],
            -1,
        ),
    ]
    ...
    fields = [
        fields[0][:, :, 1].reshape((-1, 3)),
        fields[1][:, 1, :].reshape((-1, 3)),
        fields[2][:, 1, :].reshape((-1, 3)),
        fields[3][1, :, :].reshape((-1, 3)),
        fields[4][1, :, :].reshape((-1, 3)),
    ]
    coords = [
        coords[0][:, :, 1].reshape((-1, 3)),
        coords[1][:, 1, :].reshape((-1, 3)),
        coords[2][:, 1, :].reshape((-1, 3)),
        coords[3][1, :, :].reshape((-1, 3)),
        coords[4][1, :, :].reshape((-1, 3)),
    ]
...
def _compute_fields(coords, cube_shape, b_n, batch_size=2048, progress=False):
    ...
    with torch.no_grad():
        ...
        potential = []
        iter = DataLoader(
            TensorDataset(flat_coords), batch_size=batch_size, num_workers=2
        )
